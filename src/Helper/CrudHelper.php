...
class CrudHelper
{
    ...
    public function paginatorLinks(AbstractPaginator $paginator, $routeName, $routeParams, $options)
    {
        ...
        $resolver->setAllowedValues('buttons', ['text', 'image']);
        $resolver->setAllowedTypes('use_bootstrap', 'bool');
        $resolver->setAllowedValues('bootstrap_size', ['lg', 'sm', null]);
        ...
        if ($paginator->haveToPaginate()) {
            ...
            $limit = ('sliding' == $options['type']) ? $paginator->getPage(
    ...
    protected function elementPaginatorLinks(
        ...
        $url = $this->router->generate(
            $routeName,
            array_merge($routeParams, [$options['attribute_page'] => $page])
        );
        if ('page' == $elementName) {
            if ($current) {
                $class = ($options['use_bootstrap']) ? 'active' : 'pagination_current';
                if ($options['use_bootstrap']) {
                    $content = $this->listePrivateLink($page, '', ['onclick' => 'return false;']); //bootstrap expects link
                } else {
                    $content = $page;
                }
            } else {
                $class = ($options['use_bootstrap']) ? '' : 'pagination_no_current';
                $content = $this->listePrivateLink($page, $url, [], $options['ajax_options']);
            }
        } else {
            $class = $options['buttons'].' '.$elementName;
            $buttonName = $options['buttons'].'_'.$elementName;
            $button = $options[$buttonName];
            if ('text' == $options['buttons'] || $options['use_bootstrap']) {
                $content = $this->listePrivateLink($button, $url, [], $options['ajax_options']);
            } else {
                $image = $this->util->tag('img', ['src' => $button, 'alt' => $elementName]);
                $content = $this->listePrivateLink($image, $url, [], $options['ajax_options']);
            }
        }
