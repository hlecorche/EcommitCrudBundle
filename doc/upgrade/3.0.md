...
* Les classes implémentant `SearcherInterface` sont maintenant automatiquement exclues du conteneur de services
  (lorsque l'autoconfiguration est activée). Une classe Searcher ne peut donc plus être injectée ni autowirée : elle doit
  être instanciée avec `new`. Pour utiliser un service dans une classe Searcher, passez-le par
  [les options du formulaire de recherche](../cookbook/advanced-searcher.md#définir-les-options-du-formulaire-de-recherche).
